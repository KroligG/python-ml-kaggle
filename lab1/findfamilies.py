...
class ParsedName(object):
    ...
    def __init__(self, last, title, main, nick, other):
        ...
        self.title = title
        self.main = main
        self.first = main.split()[0] if main else None
        self.nick = nick
...
class Person(graphlib.Node, DotIDMixin):
    ...
    def __init__(self, attributes, survived):
        ...
        self.id = attributes._asdict().get("passengerid")
        if self.id: self.id = int(self.id)
        self.survived = 1 if survived else 0
        self.a = attributes
...
def construct_family_components(train=None,
                                test=None,
                                tune=True):
